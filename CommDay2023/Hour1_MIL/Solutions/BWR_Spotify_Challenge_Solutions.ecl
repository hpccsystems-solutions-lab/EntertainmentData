...
//Sort by "TopGenre"
SortTopGenre := SORT(SpotMusic, TopGenre);
...
garageRock := SpotMusic(TopGenre = 'garage rock');
...
getPrince := SpotMusic(artist = 'Prince' AND year = 1984);
...
getTemptation := SpotMusic(Title = 'Into Temptation');
...
SortingSongs := SORT(SpotMusic, Artist, Title);
...
leastPop := MIN(SpotMusic, Popularity);

//Filter dataset for the leastPop value
getLeastPop := SpotMusic(leastPop = Popularity);
...
//Challenge: 
//Display all songs produced by "Coldplay" AND has a 
...
getColdPlay := SpotMusic(Artist = 'Coldplay' AND Popularity >= 75);
...
getLenSpeech := SpotMusic(SongDuration > 200 AND 
...
Song_Layout xform (SpotMusic Li) := TRANSFORM
    SELF := Li;
END;

//PROJECT
SingIt := PROJECT(SpotMusic,xform(LEFT));
...
OUTPUT(CORRELATION(SpotMusic, Popularity, Liveness), NAMED('Liveness'));
OUTPUT(CORRELATION(SpotMusic, Energy,     Loudness), NAMED('Energy'));
...
//Build transfrom
Funky_Layout xFunk (SpotMusic Fi) := TRANSFORM
