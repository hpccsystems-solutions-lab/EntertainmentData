...
NewStormRec := RECORD
 UNSIGNED4 RecID;
 STRING2   State;
 STRING10  damage_propnum;
 StormRec;
END;

SeqRecs := PROJECT(Storm,TRANSFORM(NewStormRec,SELF.RecID := COUNTER,
                                               SELF.damage_propnum := STD.str.filter(left.damage_property,'1234567890');  
                                               SELF.State := '',
                                               SELF := LEFT)); //Sequence by COUNTER
OUTPUT(SeqRecs);                                               
